import { DownloadIcon, ImagesIcon, Loader2Icon } from "lucide-react";
        ...
        <div className={`relative overflow-hidden ${aspectClasses[aspectRatio]}`}>
            ...
            {!isLoading && thumbnail?.image_url && (
                <div className="group relative h-full w-full">
                    ...
                    transition-opacity group-hover:opacity-100">
                        <button onClick={onDownload} type="button" className="mb-6 flex items-center gap-2 rounded-md px-5 py-2.5 
                        text-xs font-medium transition bg-white/30 ring-2 ring-white/40 backdrop-blur 
                        hover:scale-105 active:scale-95"><DownloadIcon className="size-4"/>
                        Download Thumbnail
                        </button>
                    </div>
                </div>
            ...
            {!isLoading && !thumbnail?.image_url && (
                ...
                border-dashed border-white/20 bg-black/25">
                    ...
                    <ImagesIcon className="size-10 text-white opacity-50"/>
                    </div>
                    <div className="px-4 text-center">
                        <p className="font text-zinc-200">Generate your first Thumbnail</p>
                        <p className="mt-1 text-xs text-zinc-400">out the form and click generate</p>
